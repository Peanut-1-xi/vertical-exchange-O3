...
HEFEI_ROOT = BASE_DIR / "\u5408\u80a5\u79d1\u5b66\u5c9b_ERA5\u6570\u636e\u4e0e\u5904\u7406\u7ed3\u679c\u6c47\u603b"
OUT_DIR = HEFEI_ROOT / "\u5408\u80a5\u79d1\u5b66\u5c9b_ERA5_2025_pressure_levels_600_1000"
...
# Hefei Science Island / HFIPS is around 31.91 N, 117.16 E.
# Keep the same 0.75 x 0.75 degree extraction size used for Shanghai.
AREA = [32.25, 116.75, 31.50, 117.50]
...
def state_file_for(account: str) -> Path:
    return OUT_DIR / f"era5_2025_hefei_science_island_600_1000_{account}_state.csv"
...
def month_range_for(account: str) -> tuple[int, int]:
    # Split March-December evenly across the two accounts.
    if account == "secondary":
        return 3, 7
    return 8, 12
...
def target_file(month: int, start_day: int, end_day: int) -> str:
    return f"era5_pressure_600_1000_hefei_science_island_2025{month:02d}_{start_day:02d}-{end_day:02d}.nc"
            ...
            rows.append(
                {
                    ...
                    "updated_at": "",
                    "note": f"{account} account queue for Hefei Science Island 600-1000 hPa pressure levels.",
                }
            )
    return rows
...
def submit_pending(client: Client, rows: list[dict[str, str]], max_active: int) -> None:
    ...
    for row in rows:
        ...
        try:
            ...
            row["updated_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            row["note"] = "Submitted by Hefei Science Island 600-1000 hPa queue script."
            active += 1
...
def main() -> None:
    ...
    while True:
        ...
        if done:
            print(f"All Hefei Science Island 600-1000 hPa periods are downloaded for account={args.account}.")
            return
        if not args.watch:
