...
def read_city(path: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df["o3_turbulent_flux_eff_upward_ug_m2_h"] = (
        pd.to_numeric(df["o3_turbulent_flux_eff_upward_ug_m2_s"], errors="coerce") * SECONDS_PER_HOUR
    )
    df["o3_advective_flux_upward_ug_m2_h"] = (
        -pd.to_numeric(df["o3_advective_flux_downward_ug_m2_s"], errors="coerce") * SECONDS_PER_HOUR
    )
...
def draw(ax, y, data_p, data_c, xlabel, xlim=None, zero_line=False):
    offset = 0.18
    width = 0.30
    bp_p = ax.boxplot(
        data_p,
...
def plot_city(config: dict[str, object], flux_col: str, flux_label: str, out_name: str, title_flux_name: str) -> Path:
    df, daily = read_city(config["input"])
    polluted_dates = set(daily[daily["polluted"]]["bjt_date"])
    flux_polluted, flux_clean = build_box_data(df, polluted_dates, flux_col)
    ...
    flux_limit = max(100.0, np.nanpercentile(np.abs(flux_all), 98) * 1.25)
    draw(
        axes[0],
        y,
        flux_polluted,
    ...
    fig.suptitle(
        f"Annual {title_flux_name} by O3 Exceedance Days, {config['label']}\n"
        f"Polluted: 0-100 m daily maximum O3 > {O3_THRESHOLD_UG_M3:.0f} ug/m3; flux unit converted to per hour",
        fontsize=14,
        fontweight="bold",
        y=0.995,
    )
...
def main() -> None:
    ...
    for config in CITY_CONFIGS:
        outputs.append(
            plot_city(
                config,
                "o3_turbulent_flux_eff_upward_ug_m2_h",
                "F_eff (+up / -down)\n(ug m-2 h-1)",
                "annual_signed_effective_flux_o3_boxplot_hourly_unit.png",
                "Signed Effective Turbulent O3 Flux",
            )
        )
        outputs.append(
            plot_city(
                config,
                "o3_advective_flux_upward_ug_m2_h",
                "F_adv (+up / -down)\n(ug m-2 h-1)",
                "annual_signed_advective_flux_o3_boxplot_hourly_unit.png",
                "Signed Vertical Advective O3 Flux",
            )
        )
